function [resode,respde] = senst(mode,F,ploton,dispon)
% SENS simule un transfert RD+RK+RH lors d'un contact solide liquide avec des isothermes arbitraires (BET, GP...)
%   Syntax: [resode,respde] = senst(mode,F[,ploton,dispon])
%i;// liq : le fluide, charge initialement avec la concentration C0liq mol/m^3 initiale, et Cinfliq mol/m^3 a la fin.
%i;// sol : le fluide, charge initialement avec la concentration C0sol mol/m^3 initiale, et Cinfsol mol/m^3 a la fin.
...
% Definitions
bet         = inline('CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub','aw','CsitessurC0sol','c','Kini','Ub'); % s = scale factor
GP          = inline('PvsatsurRTC0sol*aw-Kini-Ub','aw','PvsatsurRTC0sol','bidon','Kini','Ub'); % PvsatsurRTC0sol = scale factor, 4e-4 ...
xdefault	= mesh1D(50,[0 1],[.4 .2]);
options		= odeset('RelTol',1e-5,'AbsTol',1e-6,'Stats','yes','Initialstep',1e-8,'Maxstep',.1,'Maxorder',4);
Fdefault	= 	struct(...
				'x'			, 	xdefault,...
				'Bi'		, 	1e3,...	Biot [hm.L1/D]
				'iso'		,	struct('S',struct('conc',bet,'s',1,'c',10,'Kini',1.6/2e4),...
                                       'L',struct('conc',GP,'s',4e-4,'c',10,'Kini',1.6/20000)),...
				'L'			,	1/100,...	dimensionless thickness [L1/L2]
				'C0'		,	1,...	dimensionless initial concentration [] , C0sol-C0liq, normalisé comme on veut ici.
...
% compatibilite avec vieux matlab
false=0;
true=1;
...
switch lower(mode), case {'both' 'pde'}
    ...
    if ~errpde
        ...
        fi		= interp1((F.t(1:end-1)+F.t(2:end))/2,(fc(2:end)-fc(1:end-1)).*(F.t(2:end)-F.t(1:end-1)),ti,method);
...
switch lower(mode), case {'both' 'pde'}
    ...
    if nargout>1
        if ~errpde
            ...
            switch lower(mode)
            case 'pde', respde = senst('pde',F,ploton);
...
awb=.4;
bet         = inline('CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub','aw','CsitessurC0sol','c','Kini','Ub'); % s = scale factor
% [awb,erreur,exitstatus] = fminbnd(@bet,0,1,[],S.iso.S.s,S.iso.S.c,S.iso.S.Kini,Ub);
[awb,erreur,exitstatus] = fminbnd(inline('(CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub).^2','aw','CsitessurC0sol','c','Kini','Ub'),0,1,[],S.iso.S.s,S.iso.S.c,S.iso.S.Kini,Ub);
%i;// [awb,erreur,exitstatus] = fminbnd(inline('CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub','aw','CsitessurC0sol','c','Kini','Ub'),0,1,[],.5,15,1e-4,1);
%i;// feval(inline('CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub','aw','CsitessurC0sol','c','Kini','Ub'),.52815292415,.5,15,1e-4,1)
%i;// 
%i;// bet=inline('(CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub).^2','aw','CsitessurC0sol','c','Kini','Ub');
%i;// % c est bien une fonction croissante de [0 1] dans [0 infinity], mais il y a un bug si on vire le .^2 ... 
%i;// feval(bet,.52815292415,.5,15,1e-4,1)
%i;// % ans = 3.1344e-11 % donc l'awb devrait etre .52815292415 !!!
%i;// [awb,erreur,exitstatus] = fminbnd(bet,0,1,[],.5,15,1e-4,1)
%i;// % awb = 6.6107e-05, erreur = -0.9996, exitstatus = 1 : donc fminbnd pense avoir trouve, or c est faux !!
%i;// if abs(erreur)>.01
%i;//   error('erreur dans matlab, fminbnd deconne ...')
%i;// end
%i;// 
%i;// 
%i;// [awb,normerreur,erreur,exitstatus] = lsqcurvefit(inline('CsitessurC0sol.*aw.*c./((1-aw).*(1+aw*(c-1)))-Kini-Ub','aw','CsitessurC0sol','c','Kini','Ub'),awb,S.iso.S.s,Ub,0,1,[],S.iso.S.c,S.iso.S.Kini)
   %i;// [res,err]=lsqcurvefit(@evalueparams,zeros(nbparam,1),0,0,[],[],optimset('Display','off','TolX',0.1));
CLi = S.iso.L.conc(awb,S.iso.L.s,S.iso.L.c,S.iso.L.Kini,0); % concentration dans le fluide a la frontiere
%i;// disp([t,Ub,awb,CLi])
%;// disp(S.iso.S.conc),disp([S.iso.S.s,S.iso.S.c,S.iso.S.Kini,0])
%;// disp(S.iso.L.conc),disp([S.iso.L.s,S.iso.L.c,S.iso.L.Kini,0])
%;// disp(S.iso.S.conc([.01:.01:.99],S.iso.S.s,S.iso.S.c,S.iso.S.Kini,0))
%;// disp(S.iso.L.conc([.01:.01:.99],S.iso.L.s,S.iso.L.c,S.iso.L.Kini,0))
