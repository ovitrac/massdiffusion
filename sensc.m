...
Fdefault.iso.helperfunctions.isothapprox=@isothapprox;
Fdefault.functions.value_t_x_param=@sensca;
Fdefault.functions.mean_timet_param=@senscam;
Fdefault.functions.moinsflux_timet_param=@dsenscamsurdt;
Fdefault.functions.deriv_moinsflux_timet_param=@ddsenscamsurdtdt;
Fdefault.functions.derivseconde_moinsflux_timet_param=@dddsenscamsurdtdtdt;
method		= 'cubic';
Bi_critode	= 7e4;
Bi_maxode	= 1e18;
BiKminode	= 0.1;
...
%i;// Solution
if any(strcmpi(methode,{'all' 'analytic'}))
    ...
    piso=find(F.iso.S.Cs<cs0, 1, 'last' );
    ...
    nmax=ceil(1+nblastterms+10*log(1+1/moneps)/sqrt(min(Fdefault.t(Fdefault.t>0))));
	...
	tplot=log(eps^.5+F.t);
	yplot=eventoccurs(tplot,segment,cspplus1);
	if ploton
	  figure(12)
	  clf
	  ...
	  if meth(5)<double('a')
	    plot(tplot/log(10),eventoccurs(tplot,segments_energy(p),cspplus1)+cspplus1,'g')
	    plot(tplot/log(10),log10(eps^5+abs(eventoccurs(tplot,segments_energy(p),cspplus1)-yplot)),'r')
	...
	if max(diff(yplot))>0
	  ...
	  noerror=true;
	  if min(yplot(diff(yplot)>0))<0 && max(yplot(diff(yplot)>0))>0
	    error('this is at a crossing point')
	    noerror=false;
	  end
	  if min(yplot(diff(yplot)>0))<segment.csp-cspplus1 && max(yplot(diff(yplot)>0))>segment.csp-cspplus1
	    error('l isotherme remonte un coin.')
	    noerror=false;
	  end
	  if noerror
	    warning('Ouf, pas d''erreur causée par cette limitation.')
	  end
	end
	%i;// avec l'arobas de matlab, ce serait plus propre, mais infaisable en scilab, et en vieux matlab.
	%i;// struct.cs1=0; [logt1,err]=lsqcurvefit(@eventoccurs,0,segment,cs1,[],[],optimset('Display',DoDisplay)); est une version plus lente et plus longue de la ligne suivante.
	%i;// partir de 0 plante lorsque c'est trop plat, genre lorsque biot est grand.
	[logt1,err]=fsolve(@eventoccurs,tplot(max([1;find(yplot(:)>0)])),optimset('Display',DoDisplay),segment,cspplus1);
	...
	if meth(5)<double('a')
	  [logt1_energy,err]=fsolve(@eventoccurs,0,optimset('Display',DoDisplay),segments_energy(p),cspplus1);
    ...
    resanalytic.functions.deriv_moinsflux_timet_param=@ddsenscamsurdtdt;
	...
	departetdautre=departetdautre(departetdautre>0);
      end
    ...
    if any(strcmpi(methode,{'analytic'}))
      resode=resanalytic;
      resode.Cx=[];
      resode.parametres=[];
...
if any(strcmpi(methode,{'all' 'odepde' 'pde'}))
...
if any(strcmpi(methode,{'odepde' 'ode'}))
...
avecresode=0;
avecrespde=0;
if any(strcmpi(methode,{'all' 'odepde' 'ode'}))
	if nargout>0
		resode.t = ti;
		resode.C = Codei;
		resode.f = fodei;
...
if any(strcmpi(methode,{'all' 'odepde' 'pde'}))
  if nargout>0 && strcmpi(methode,'pde'), resode	= []; end
  if nargout>1
    if ~errpde
    else
      switch lower(methode);%i;select lower(methode);// compatibilité scilab matlab.
...
if ploton
	...
	if any(strcmpi(methode,{'all' 'odepde'}))
		figure(7)
...
%i;endfunction
function [res,grad,cossin]=G1(x,LsurKp,unsurBiKp,n) % TODO noter que les dérivées ne servent pas à fzero
...
function res=A_n0__p(lambda_np,lambda_np_cache,Bi,Kp,L)
  res=L./(lambda_np.*lambda_np_cache.sin/Bi+Kp.*lambda_np.*(.5./lambda_np_cache.sin-lambda_np_cache.cos./lambda_np./2));% FAIT20130221 pourquoi c'est L et non (L+Kp) ici ????? en fait c'est toujours renomalisé à chaque utilisation de A_n0__p, tout va bien.
...
function res=A_n0__psurL(lambda_np,lambda_np_cache,Bi,Kp,L)
  ...
  %i;// NB: c'est normal qu'on ne puisse pas passer à la limite pour lambda qui tend vers 0.
%i;endfunction
function [res,lambdan0_cache,nlarge]=lambda_n__p(Bi,Kp,L,nmax,varargin)
%i;// calcule la liste des valeurs propres.
  ploton=false;
  first=1;%i;first=[]
  if ~isempty(varargin) ;arg=varargin(1); ploton=[arg{first}]; end;
  ...
  for i=1:10;
    xqminus1=xq;
    %i;// avec acotan, ca aurait été plus stable, notamment car les lambdas sont tous entre 0 et pi, mais TODO comme alors xq tend vers0, ca aurait été toucher les bornes maximums de cotan, et alors que peut se passer ??? ah, si j'avais une fonction atancotan, ce serait l'idéal. Ah j'ai trouvé, c'est atan2(r*sin(t),r*cos(t)) qui renvoie t pour r>0 et -pi<t<pi.
    xq=atan2(Bi*Kp*(alln*pi+xqminus1),(alln*pi+xqminus1).^2-Bi*L);
    if any(xq<0)
      error('erreur bug35 dans lambda_n__p, xq aurait du rester positif car y est positif dans atan2(y,x)')
      xq(find(xq<0))=xq+pi;
    end
    %i;// xq=atan(Bi*Kp*(alln*pi+xq)./((alln*pi+xq).^2-Bi*L));
    %i;// xq(find(xq<0))=xq+pi;
    if ploton
      subplot(311)
      plot (log10(1:nmax+2),log10(eps^10+(eps^5+abs(xq-xqminus1))./abs(xq)))
      hold on
      subplot(312)
      plot (log10(1:nmax+2),xq)
      hold on
      subplot(313)
      plot (log10(1:nmax+1),diff(xq))
      hold on
      drawnow
    end
  end
  ...
  %i;// matlab7 optimise et transforme nlarge=min(nmax+2,max(find(abs(xqminus1-xq)>eps^.9*abs(xq)))+5) en nlarge=min(nmax+2,max(find(abs(xqminus1-xq)>eps^.9*abs(xq))+[])) de temps en temps ...
  nlarge=find(abs(xqminus1-xq)>eps^.9*abs(xq), 1, 'last' );
  %i;// xq(find(xq<0))=xq(find(xq<0))+pi;
  if isempty(nlarge)
    nlarge=0;
  end
  nlarge=min(nmax+2,nlarge+5);
  arefaire=1:nlarge;
%i;//   lambdaplusdemipiDIVpiarefaire(2).s=[lambdaplusdemipiDIVpiarefaire(1).s(find((alln(lambdaplusdemipiDIVpiarefaire(1).s)*pi+pi/2+3*pi).^2>Bi*L));unique(max(1,[0:3]+floor(sqrt(Bi/L)/pi))];
%i;//   lambdaplusdemipiDIVpiarefaire(1).s=[lambdaplusdemipiDIVpiarefaire(1).s(find((alln(lambdaplusdemipiDIVpiarefaire(1).s)*pi-pi/2-3*pi).^2<Bi*L));unique(max(1,[-1:2]+floor(sqrt(Bi/L)/pi))];
  for nolambdan0=arefaire;
    minerr=eps.^(-5);
    optres=-1;
    maxapresdemipi=0+(alln(nolambdan0)<ceil(sqrt(Bi*L)/pi));
    apresdemipi=min(nolambdan0*4-7,1-(alln(nolambdan0)+1>floor(sqrt(Bi*L)/pi)));
...
%i;// 	n=alln(nolambdan0)+0*xlist;
%i;// 	[ylist,dysurdx,cossin]=G1(xlist,L/Kp,1/Bi/Kp,alln(nolambdan0));
%i;// 	liste0=contourc(log10(xlist),[1 2],[1 1]'*ylist,[0 0]);
%i;// 	liste0=[xq(1),xqminus1(1),10.^liste0(1,2:3:end)];
    dodisplay='off';if ploton;dodisplay='on';end;
    %i;// TODO, attention avec matlab v6, fzero est tres tres lent ...
    [optres,minerr]=fzero(@G1,[eps,pi],optimset('Display',dodisplay,'TolX',eps^5),L/Kp,1/Bi/Kp,alln(nolambdan0)); % autre version de matlab dans www.mathworks.com/help/matlab/ref, il y a aussi www.mathworks.com/help/stat/ref/chi2inv.m
    apresdemipi=maxapresdemipi+2;
    while apresdemipi<=maxapresdemipi % DG20130130: toute la boucle a été commentée par la ligne précédente.
      if apresdemipi==-3
	%i;// [1,x].*[x+n*pi;LsurKp-unsurBiKp*(x+n*pi).^2];
	xlist=[10.^[log10(eps):.1:log10(pi)],pi];
	n=alln(nolambdan0)+0*xlist;
	[ylist,dysurdx,cossin]=G1(xlist,L/Kp,1/Bi/Kp,alln(nolambdan0));
	liste0=contourc(log10(xlist),[1 2],[1 1]'*ylist,[0 0]);
	liste0=[xq(1),xqminus1(1),10.^liste0(1,2:3:end)];
	if ploton
	  figure(24);
	  subplot(221)
	...
	if isempty(liste0)
	  ...
	  if etendu==1
	    ...
	    lambdamin=max([0,xlist(xlist<xinitial)]);
	    lambdamax=min([pi,xlist(xlist>xinitial)]);
	    liste0=liste0(2:end);
	  end
	end
      end
	...
	xinitial=xq(nolambdan0);
	apresdemipi=apresdemipi+1;
      end
      if apresdemipi==0;
	xinitial=sqrt((L/Kp-1)*Bi*Kp)-alln(nolambdan0)*pi;
...
%i;//       end
	%i;// try
	  [maxXmh,err]=fzero(@G1,[lambdamin,lambdamax],optimset('Display',dodisplay,'TolX',eps^5),L/Kp,1/Bi/Kp,alln(nolambdan0));
%i;//	  [maxXmh,err]=lsqcurvefit(@G1,xinitial,L/Kp,0,lambdamin,lambdamax,optimset('Display',dodisplay,'Jacobian','on','DerivativeCheck','off'),1/Bi/Kp,alln(nolambdan0));
%i;//	  if ploton
%i;//	    figure(24);
%i;//	    subplot(221)
...
%i;//	  end
%i;//	  [maxXmh,err]=lsqcurvefit(@G1,maxXmh,L/Kp,0,lambdamin,lambdamax,optimset('Display',dodisplay,'TolFun',eps^5,'TolX',eps^5,'MaxIter',44,'Jacobian','on','DerivativeCheck',dodisplay),1/Bi/Kp,alln(nolambdan0));
	  if ploton
	    figure(24);
	    subplot(221)
	...
	xinitial=rand(1,1)*(lambdamax-lambdamin)+lambdamin;
      ...
      %i;// TODO ou faire encore mieux, avec xq=arctan(fractionde{\[ \tan (\lambda ) = \frac{{{\text{Bi}}K_1 \lambda }} {{\lambda ^2  - {\text{Bi}}L}} \]}([0:nmax+1]+xq)) repete deux ou trois fois, en partant de 0, et le tour est joue !
    end
    if optres==-1
...
%i;//    end
  ...
  res=res(1:nmax);
  lambdan0_cache.modpi=xq(1:nmax);
...
%i;endfunction
function [res,gradient]=eventoccurs(logt,segment,cspplus1)
...
%i;// analytic solution evaluated by user.
  ...
  for not=1:size(t,1)
    p=find(list2vec([segments.tp])<t(not), 1, 'last' );
    segment=segments(p);
    for nox=1:size(x,1)
...
%i; endfunction
function isoc=approximate_isotherm(iso,varargin) % varargin: nbinter, ploton,
  %i;// isoc: tangeant en min(awminawmax), aw0flu, csfinal, csinit, max(awminawmax) ; pour le reste, équi c-s, avec cordes dont les points de controle sont rapprochés d'un facteur sqrt(abs(nbinter(2))-3) pouvant aller de 0 (tangentes) à 1 (lagrange d'ordre 1), en passant par 1/sqrt(3) qui est le meilleur approximant.
  first=1;%i;first=[]
  nbinter=0; if ~isempty(varargin);arg=varargin(1);arg=[arg{first}];if ~isempty(arg); nbinter=arg;end;end;
  ...
  if abs(nbinter(1))>0;
	...
	isoc.Cs=real([isoc.Cs(1:length(isoc.Cs)-1),cs(nonbs):diff(cs(nonbs:nonbs+1))/nbs(nonbs):cs(nonbs+1)]);
      ...
      isoc.aws=aw;
      if abs(nbinter(2))>=3;
	%i;// si nbinter(2)>3 ou nbinter(2)<-3, on n utilise plus de derivee troisieme, a la place on utilise des cordes basées sur des distances plus courtes d'un facteur sqrt(abs(nbinter(2))-3).
	Csp=isoc.Cs+(1-sqrt(abs(nbinter(2))-3))*[diff(isoc.Cs),0]/2;
	Csm=isoc.Cs-(1-sqrt(abs(nbinter(2))-3))*[0,diff(isoc.Cs)]/2;
	awsp=betiso.invconc(Csp,betiso);
	awsm=betiso.invconc(Csm,betiso);
	%i;// pour tous les points . .
	ind=1:sum(nbs)+1;
	%i;// . . intérieurs qui ne sont pas des points de controle . .
	ind(cumsum([1,nbs]))=[];
	%i;// ind([1,1+sum(nbs)])=[]; debug only !
	%i;// . . on calcule l'intersection des segments, cf notes 220808 . .
	df=[diff([Csm(ind+1);Csp(ind);Csm(ind);Csp(ind-1)]);diff([awsm(ind+1);awsp(ind);awsm(ind);awsp(ind-1)])];
	dCs=real((df(1,:).*df(5,:)-df(2,:).*df(4,:))./(df(1,:).*df(6,:)-df(3,:).*df(4,:)));
	%i;// . . lorsque les intersection sont bien la ou on pense . .
	indf=find(dCs>0);
	isoc.Cs(ind(indf))=Csm(ind(indf))+dCs.*(Csm(ind(indf))-Csp(ind(indf)-1));
	isoc.aws(ind(indf))=awsm(ind(indf))+dCs.*(awsm(ind(indf))-awsp(ind(indf)-1));
	%i;// . . sinon, on le met calé au . .
	indf=find(dCs<0);
	isoc.Cs(ind(indf))=Csm(ind(indf));
	isoc.aws(ind(indf))=awsm(ind(indf));
	%i;// . . bord qui convient pour etre dans l intervalle [Csm,Csp].
	indf=find(isoc.Cs(ind)>Csp(ind));
	isoc.Cs(ind(indf))=Csp(ind(indf));
	isoc.aws(ind(indf))=awsp(ind(indf));
	if ploton
	  figure(84);
	  hold on;
	  plot(isoc.aws,isoc.Cs);
	  plot(awsm,Csm,'+g');
	  plot(awsp,Csp,'+b');
	end
      end
	...
	dCs=real((df(1,:).*df(4,:)-df(2,:).*df(3,:))./(df(4,:)-derprim.*df(2,:)));
	indf=find(~(dCs>0&dCs<df(1,:)+df(2,:)&isfinite(dCs)));
	...
	ind=cumsum(nbs);
	derprim=1./betiso.derconc(isoc.aws(ind+1),betiso);
	...
	error('tangentes a programmer bug64983.')
      end
    end
    ...
    imin=find(aw<=betiso.aw0flu, 1, 'last' );
...
%i;endfunction
function [Cs,approxiso,isoparfait]=isothapprox(K,nbintervalles,aw)
  %i;// fait pour la structure K choisie par fig_publiv1 pour créer les isothermes.
  if isstruct(K)
    if isfield(K,'pourcentstarch')
      isoparfait=baldev02isotherms(K.name,baldev02STARCHandPE(K.name,K.pourcentstarch/100,K));
    else
      details=K.constants;
      details.aw0flu=K.aw0flu;
      details.C0sol=K.C0sol;
      details.Ceqsol=K.Ceqsol;
      details.awminawmax=K.awminawmax;
      isoparfait=baldev02isotherms(K.name,details);
    end
  else
    isoparfait=struct('conc',isothermeacoin,'invconc',invisothermeacoin,'aw0',0,'Cs',[0 10],'K',K*[1 1]);
    warning('attention le K utilise en entrée ici est l''inverse du K du papier');
  end
  approxiso=approximate_isotherm(isoparfait,[-nbintervalles 10/3]);
  Cs=approxiso.conc(aw,approxiso);
%i;endfunction
...
%i;//  vim:softtabstop=2:shiftwidth=2:guioptions+=a
%i;//  :noremap à :execute 'w'<bar>:w! Z:\common\senscrep20080616\<c-r>%
